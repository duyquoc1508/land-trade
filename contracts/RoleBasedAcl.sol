...
contract RoleBasedAcl {
	...
	mapping(address => uint) public userAddressIndexes;
	 ...
	 */
	function addRole(address _account, Roles _role) external onlyRole(0) emptyRole(_account){
	...
	function removeRole(address _account, Roles _role) external onlyRole(0) {
		...
		require(hasRole(_account, uint8(_role)), "Roles: Account doesn't have role");
    ...
    modifier emptyRole(address _address) {
        require(!hasRole(_address, 0) && !hasRole(_address, 1),
     ...
     */
    function getAllAddressAndRole() external view  returns( address[] memory, uint8[] memory ) {
		uint8[] memory listRole = new uint8[]( userAddress.length );
		uint amountRoles = uint8(Roles.LAST_INDEX);
		for( uint8 i = 0 ; i < userAddress.length; i++ ){
            for( uint8 j = 0 ; j < amountRoles ; j++ ){ 	// number roles 2 = {SUPER_ADMIN, NOTARY}
